  ...
  return (
    <footer className="bg-[var(--navy)] text-white">
      <div className="section-shell py-7 sm:py-12 lg:py-16">
        <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr_0.7fr_1fr] lg:items-start lg:gap-10">
          ...
          <div className="mx-auto grid max-w-md grid-cols-2 gap-4 border-t border-white/10 pt-5 text-center lg:mx-0 lg:max-w-none lg:contents lg:border-0 lg:pt-0 lg:text-left">
            {footerGroups.map((group) => (
              <div key={group.title} className="min-w-0">
                <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-[var(--blue-light)] sm:text-[11px]">
                  {group.title}
                </p>
                <div className="mt-2 grid gap-1 sm:mt-3 sm:gap-2">
                  {group.links.map(([label, href]) => (
                    <Link
                      key={href}
                      href={href}
                      className="py-1 text-[13px] leading-5 text-white/65 transition duration-300 hover:text-white sm:text-sm"
                    >
                      {label}
                    </Link>
                  ))}
